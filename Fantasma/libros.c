...
*
*/
int libros_Inicializar(Libros array[], int size)                                    //cambiar fantasma
{
    int retorno=-1;
...
//            utn_getTexto("getTexto\n: ","\nError",1,TEXT_SIZE,1,array[posicion].varLongString);                 //mensaje + cambiar campo varLongString
            printf("\n Posicion: %d\n ID: %d\n titulo: %s\n",
                   posicion, array[posicion].idUnico,array[posicion].titulo);
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n titulo: %s\n",
                       array[i].idUnico,array[i].titulo);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
