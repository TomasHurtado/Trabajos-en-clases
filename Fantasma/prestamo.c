...
int prestamo_buscarString(Prestamo array[], int size, char* valorBuscado, int* indice)                    //cambiar prestamo
{
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(strcmp(array[i].fecha,valorBuscado)==0)                                        //cambiar campo varString
            {
                *indice=i;
                retorno=0;
                break;
...
//            utn_getTexto("getTexto\n: ","\nError",1,TEXT_SIZE,1,array[posicion].varLongString);                 //mensaje + cambiar campo varLongString
            printf("\n Posicion: %d\n ID: %d\n fecha: %s\n",
                   posicion, array[posicion].idUnico,array[posicion].fecha);
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n fecha: %s\n",
                       array[i].idUnico,array[i].fecha);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
