...
int getString(char* msg, char* msgError, int min, int max, int* reintentos, char* resultado)
{
    ...
    if(msg!=NULL && msgError!=NULL && min<=max && reintentos>=0 && resultado!=NULL)
    {
        do
        {
            ...
            if(strlen(bufferStr)>=min && strlen(bufferStr)<max)    // tamaño (max) =solucion de elementos (strlen) + 1(\0)
            {
...
//-----------------------------------------
int utn_getUnsignedInt(  char* msg,char* msgError,int minSize,int maxSize,int min,int max,int reintentos,int* input) // int min,int max
{
    int retorno=-1;
    int aux;
    char bufferStr[maxSize];

    if(msg!=NULL && msgError!=NULL && minSize<maxSize && max>=min && reintentos>=0 && input!=NULL)
    {
        do
        {
