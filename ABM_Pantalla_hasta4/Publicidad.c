...
{   int i;
    ...
    for(i=0;i<lenPan;i++)
    {
        ...
        {
            posPant= pan_findPanById(pPantalla,lenPan,pPublicidad[i].idPantalla);
           if(pPublicidad[i].dias<=dia)
           {
                if(posPant>=0)
                {
                   printf("nombre:%s\n precio%.2f\n dias%d\n total x dias %f ",
                                                       pPantalla[posPant].nombre,
                                                       pPantalla[posPant].precio,
                                                       pPublicidad[i].dias,
                                                       pPublicidad[i].dias*pPantalla[i].precio);
...
int pan_orderByday(Publicidad* pPublicidad, int len)//mayor
{
    int i;
    int j;
    Publicidad buffer;
    for(i=0;i<len-1;i++)
    {
        ...
        for(j=i+1;j<len;j++)
        {
            if(pPublicidad[j].isEmpty==1)
            {
                continue;
            }
            if(pPublicidad[i].dias>pPublicidad[j].dias)
            {
                buffer=pPublicidad[i];
                pPublicidad[i]=pPublicidad[j];
                pPublicidad[j]=buffer;
            }
        }
